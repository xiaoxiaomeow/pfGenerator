...
public class TaskToHtml {
    public static void taskToHtml () throws IOException {
        ...
        File target = new File("feats/html");
        // Tools.clearPath(target);
        target.mkdirs();

        JSONArray feats = JSONArray.fromObject(Tools.readFile(source));
        for (Object object : feats) {
            JSONObject feat = (JSONObject) object;
            String key = feat.getString("key");
            File targetFile = new File(target, key.toLowerCase().replaceAll("/", "-") + ".html");
            if (targetFile.exists()) continue;

            String urlString = feat.getString("url");
